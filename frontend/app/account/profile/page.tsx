...
import { Pencil, Check, X, User, Camera, Upload } from 'lucide-react'
...
export default function ProfilePage() {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 3. Reload global user state immediately
      await fetchUser()
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row gap-10">
        <div className="w-full md:w-64 shrink-0 border-b md:border-b-0 md:border-r border-gray-100 pb-8 md:pb-0 md:pr-10">
          <div className="flex flex-col items-center text-center">
            
            <div className="relative group mb-6">
              <div className={`w-36 h-36 rounded-full bg-[#F3F4F6] border-4 border-white shadow-xl flex items-center justify-center text-gray-400 overflow-hidden transition-all ${uploadingAvatar ? 'opacity-50 blur-sm' : ''}`}>
                {user.avatarUrl ? (
              ...
              <button 
                ...
                className="absolute bottom-2 right-2 p-2.5 bg-[#7C3AED] hover:bg-purple-800 text-white rounded-full shadow-lg transition-transform hover:scale-110 flex items-center justify-center"
              >
                {uploadingAvatar ? <Upload className="h-4 w-4 animate-bounce" /> : <Camera className="h-4 w-4" />}
              </button>
